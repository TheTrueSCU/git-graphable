...
def test_example_repo_html_metadata(repo_name):
    ...
    config = GitLogConfig(
        ...
        if "issue" in repo_name or "release" in repo_name or "collab" in repo_name
        else None,
        issue_engine="script"
        if "issue" in repo_name or "release" in repo_name or "collab" in repo_name
        else None,
        ...
        if repo_name == "repo-pristine":
            ...
            for n in nodes:
                ...
                hygiene_tags = [
                    t
                    for t in tags
                    if t in [Tag.WIP.value, Tag.ORPHAN.value, Tag.DIRECT_PUSH.value]
                ]
            ...
            assert any(
                "issue_inconsistency" in n["data"].get("tags", []) for n in nodes
            )
            assert "issue_inconsistency" in tag_styles

        elif repo_name == "repo-release-desync":
            assert any(
                "release_inconsistency" in n["data"].get("tags", []) for n in nodes
            )
