...
def generate_pristine():
    ...
    run_git(["merge", "--no-ff", "feature/login", "-m", "Merge pull request #1 from feature/login"], path)
...
def generate_messy():
    ...
    subprocess.run(["git", "commit", "-m", "feat: ancient work"], cwd=path, env=env, check=True)
